...
// Pure JS MD5 Implementation for browser
...
export default function HashGenerator() {
  ...
  const handleInputChange = async (val: string) => {
    ...
    try {
      ...
      const sha1Val = await subtleHash("SHA-1", val);
      const sha256Val = await subtleHash("SHA-256", val);
      const sha384Val = await subtleHash("SHA-384", val);
      const sha512Val = await subtleHash("SHA-512", val);

      setHashes([
        { name: "SHA-256", value: sha256Val },
        { name: "SHA-512", value: sha512Val },
        { name: "MD5", value: md5Val },
      ...
      <div className={styles.toolGrid}>
        <div className={styles.inputSection}>
          <div className={styles.sectionHeader}>
            <label className={styles.label}>Input String / Password</label>
          </div>
          <textarea
            className="textarea"
            value={input}
            onChange={(e) => handleInputChange(e.target.value)}
            placeholder="Type or paste any text to calculate hashes in real time..."
            spellCheck={false}
          />
          <div className="btn-group">
            <button className="btn" onClick={() => handleInputChange("Hello, ToolNest!")}>
              📄 Sample
            </button>
        ...
        <div className={styles.outputSection}>
          <div className={styles.sectionHeader}>
            <label className={styles.label}>Calculated Hashes</label>
          </div>
          {hashes.length > 0 ? (
